 ...
 public class ChunkLightSystem
 {
    ...
    private const byte MinAmbientLight = 2; // Minimum ambient light level
     ...
     private void CalculateCrossChunkSunlight(IEnumerable<ChunkEntity> chunks, Dictionary<(float, float), ChunkEntity> chunkMap, GetNeighborChunkDelegate getNeighborChunk)
     {
     ...
     private void PropagateSunlightToNeighbors(ChunkEntity chunk, Dictionary<(float, float), ChunkEntity> chunkMap, GetNeighborChunkDelegate getNeighborChunk)
     {
     ...
     private void PropagateLightAcrossBoundary(ChunkEntity fromChunk, ChunkEntity toChunk, int dx, int dz)
     {
         ...
         {
             fromStartX = ChunkEntity.Size - 1; fromEndX = ChunkEntity.Size - 1;
             toStartX = 0; toEndX = 0;
             fromStartZ = 0; fromEndZ = ChunkEntity.Size - 1;
             toStartZ = 0; toEndZ = ChunkEntity.Size - 1;
         }
         else if (dx == -1) // fromChunk is east of toChunk
         {
             fromStartX = 0; fromEndX = 0;
             toStartX = ChunkEntity.Size - 1; toEndX = ChunkEntity.Size - 1;
             fromStartZ = 0; fromEndZ = ChunkEntity.Size - 1;
             toStartZ = 0; toEndZ = ChunkEntity.Size - 1;
         }
         else if (dz == 1) // fromChunk is north of toChunk
         {
             fromStartZ = ChunkEntity.Size - 1; fromEndZ = ChunkEntity.Size - 1;
             toStartZ = 0; toEndZ = 0;
             fromStartX = 0; fromEndX = ChunkEntity.Size - 1;
             toStartX = 0; toEndX = ChunkEntity.Size - 1;
         }
         else // dz == -1, fromChunk is south of toChunk
         {
             fromStartZ = 0; fromEndZ = 0;
             toStartZ = ChunkEntity.Size - 1; toEndZ = ChunkEntity.Size - 1;
             fromStartX = 0; fromEndX = ChunkEntity.Size - 1;
             toStartX = 0; toEndX = ChunkEntity.Size - 1;
         }
     ...
     private void CalculateCrossChunkBlockLights(IEnumerable<ChunkEntity> chunks, Dictionary<(float, float), ChunkEntity> chunkMap, GetNeighborChunkDelegate getNeighborChunk)
     {
         foreach (var chunk in chunks)
         {
             var lightLevels = chunk.LightLevels;
     ...
     private void PropagateBlockLightsToNeighbors(ChunkEntity chunk, Dictionary<(float, float), ChunkEntity> chunkMap, GetNeighborChunkDelegate getNeighborChunk)
     {
         var chunkX = (int)(chunk.Position.X / ChunkEntity.Size);
         var chunkZ = (int)(chunk.Position.Z / ChunkEntity.Size);
    ...
    private void PropagateLightFromSource(ChunkEntity chunk, byte[] lightLevels, int startX, int startY, int startZ, byte startLight)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (var (nx, ny, nz) in neighbors)
            {
                ...
                if (neighborBlock != null)
                {
                    ...
                    reduction = (blockDef?.IsTransparent ?? false) ? (byte)2 : (byte)15; // Much higher reduction for solid blocks
                }
